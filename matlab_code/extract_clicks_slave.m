...
% 2016 12 24  Extract clicks on far-field array using detection on suction
%             cup array
...
if isunix
    ...
    switch trial
        case '20150426_am'
            ...
            s_folder = '/20150426/trial1_am/slave_matfiles_filt';
            s_file_pre = 'slave_20150426_102455';
        case '20150426_pm'
            m_folder = '/20150426/trial2_pm/slave_matfiles_filt';
            m_file_pre = 'master_20150426_145621';
        case '20150427_am'
            m_folder = '/20150427/trial1_am/slave_matfiles_filt';
            m_file_pre = 'master_20150427_103024';
    end
else
...
for iF=1:length(file_want_idx)
    ...
    s_fname = sprintf('%s_%04d_filt_%s',s_file_pre,file_want_idx(iF),file_post);

    disp(['Processing ',m_fname]);

    load(fullfile(base_data_folder,m_folder,m_fname));      % master data
    load(fullfile(base_data_folder,m_folder,m_det_fname));  % detection output
    S = load(fullfile(base_data_folder,s_folder,s_fname));  % slave data
    ...
    click_ex = nan(click_num,length(ch_wanted),ex_len);  % extracted master signals
    click_ex_s = nan(size(click_ex));                    % extracted slave signals
    for iC=1:click_num
        % Extract master click
        ...
        % Calc slave click approx location
